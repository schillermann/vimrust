...
use self::editor_selection_transform::{
    CamelCaseTransform, FlatCaseTransform, KebabCaseTransform, PascalCaseTransform,
    ScreamingSnakeCaseTransform, SnakeCaseTransform, TrainCaseTransform,
};
...
impl EditorSelection {
    ...
    pub(super) fn flat(
        ...
        let range = self.visual.range_for(cursor, file, line_view);
        let transform = FlatCaseTransform;
        range.apply_to(file, line_view, cursor_update, &transform);
        self.visual.clear();
    }
}
