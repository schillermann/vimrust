...
use vimrust_protocol::{
    DocumentFile, PromptInputSelection, PromptListItemFrame, PromptListSelection, PromptUiAction,
    PromptUiFrame, RequestEditorMode,
};
...
impl PromptUiState {
    ...
    pub fn prompt_command_for(&mut self, scope: CommandScope) {
        ...
        self.list_command = self.list_command.reset_selection();
    ...
    pub fn prompt_keymap(&mut self) {
        ...
        self.line.start_prompt(';');
        self.list_keymap = self.list_keymap.reset_selection();
        self.line_focus = true;
        self.history_command.reset_navigation();
    }

    pub fn line_overwrite(&mut self, new_content: String) {
        self.line.overwrite(new_content);
        self.list_reset();
        self.line_focus = true;
        self.history_command.reset_navigation();
        self.prefix_switch();
    ...
    pub fn command_text(&self) -> &str {
        self.line.snapshot().content()
    }

    pub fn history(&self) -> DocumentFile {
        self.history_command.file()
    }

    pub fn line_selection(&self) -> PromptInputSelection {
        self.line.snapshot().selection()
    }
    ...
    pub fn prompt_mode_sync(&self, editor_mode: &mut EditorMode, path: &DocumentFile) {
    ...
    pub fn apply_action(&mut self, action: PromptUiAction, list_rows: usize) {
        match action {
            ...
            PromptUiAction::Complete => {
                let snapshot = self.line.snapshot();
            ...
            PromptUiAction::SelectFromList => {
                ...
                let selected_label = {
                    let snapshot = self.line.snapshot();
                    let matches = self.list_filter(snapshot.content());
                    if self.line_focus || matches.is_empty() {
                        String::new()
                    } else {
                        let selection = self.list_selection().clamped(matches.len());
                        let empty_index = PromptListSelection::empty().index();
                        let index = selection.index();
                        if index == empty_index || index >= matches.len() {
                            String::new()
                        } else {
                            matches[index].label().to_string()
                        }
                    }
                };
                if !selected_label.is_empty() {
                    let line = format!(":{}", selected_label);
                    let placeholder = PromptInputPlaceholder;
                    let selection = placeholder.selection_for(&line);
                    self.line.overwrite_with_selection(line, selection);
                    self.line_focus = true;
                    self.history_command.reset_navigation();
                    self.prefix_switch();

                    let snapshot = self.line.snapshot();
                    let updated_matches = self.list_filter(snapshot.content());
                    let mut updated_index = usize::MAX;
                    let mut idx = 0;
                    while idx < updated_matches.len() {
                        if updated_matches[idx].label() == selected_label {
                            updated_index = idx;
                            break;
                        }
                        idx += 1;
                    }
                    if updated_index != usize::MAX {
    ...
    fn move_selection(&mut self, action: PromptUiAction, list_rows: usize) {
        ...
        let empty_index = PromptListSelection::empty().index();
        let current_index = selection.index();
        if current_index == empty_index {
            match action {
    ...
    fn prefix_switch(&mut self) {
        let snapshot = self.line.snapshot();
        let mut chars = snapshot.content().chars();
        match chars.next() {
    ...
    fn list_selection(&self) -> PromptListSelection {
        match self.prompt_kind {
    ...
    fn list_selection_clear(&mut self) {
        match self.prompt_kind {
            PromptKind::Command => {
                self.list_command = self.list_command.selection_clear();
            }
            PromptKind::Keymap => {
                self.list_keymap = self.list_keymap.selection_clear();
            }
        }
    }
    ...
    fn list_select_index(&mut self, new_index: usize) {
        match self.prompt_kind {
            PromptKind::Command => {
                self.list_command = self.list_command.select_index(new_index);
            }
            PromptKind::Keymap => {
                self.list_keymap = self.list_keymap.select_index(new_index);
            }
        }
    }
    ...
    fn list_scroll_reconcile(&mut self, visible_rows: usize) {
        match self.prompt_kind {
            PromptKind::Command => {
                self.list_command = self.list_command.adjust_scroll_for_visible_rows(visible_rows);
            }
            PromptKind::Keymap => {
                self.list_keymap = self.list_keymap.adjust_scroll_for_visible_rows(visible_rows);
            }
        }
    }
}
...
pub(crate) struct CommandUiView<'a> {
    ...
    pub(crate) selection_index: PromptListSelection,
